  ...
  if (isset($_POST["name"])) {
    ...
    while (list($id,$val) = each ($_POST["name"])) {
      if (!$id && isset($_POST["name"][0]) && $_POST["name"][0] != "") {
        ...
        $tables["attribute"],sql_escape($_POST["name"][0]),sql_escape($_POST["type"][0]),$_POST["listorder"][0],sql_escape($_POST["default"][0]),$_POST["required"][0],$lc_name);
        Sql_Query($query);
...
#          Sql_Query('insert into '.$table_prefix.'listattr_'.$lc_name.' (name) values("Unchecked")');
        ...
        switch ($existingtype) {
          case "textline":case "hidden":case "date":
          	 print "Converting ".htmlentities($_POST["name"][$id])." from $existingtype to ".htmlentities($_POST["type"][$id])."<br/>";
...
#                  }
          ...
          case "checkboxgroup":
            if ($_POST["type"][$id] == "hidden" || $_POST["type"][$id] == "textline") {
		         	 print $GLOBALS['I18N']->get('converting')." ".htmlentities($_POST["name"][$id])." ".$GLOBALS['I18N']->get('from')." $existingtype ".$GLOBALS['I18N']->get('to')." ".htmlentities($_POST["type"][$id])."<br/>";
        ...
        $tables["attribute"],sql_escape($_POST["name"][$id]),sql_escape($_POST["type"][$id]),$_POST['listorder'][$id],sql_escape($_POST['default'][$id]),$_POST['required'][$id],$id);
        Sql_Query($query);
      }
    }
